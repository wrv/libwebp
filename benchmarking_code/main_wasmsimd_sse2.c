#include "decode_webp_wasmsimd_sse2.h"
#include "helpers.h"
...
int main(int argc, const char** argv) {
  ...
  w2c_decode__webp__wasmsimd__sse2 inst = { 0 };

  /* Construct the module instance. */
  wasm2c_decode__webp__wasmsimd__sse2_instantiate(&inst);

  wasm_rt_memory_t* mem = w2c_decode__webp__wasmsimd__sse2_memory(&inst);
  ...
  u32 webp_file = w2c_decode__webp__wasmsimd__sse2_malloc(&inst, data_size);
  // Copy data to sandbox memory
  memcpy(&(mem->data[webp_file]), data, data_size);

  /////////////////////////////////////////////////////////////////////
  // Get an address to which config can be set
  u32 config = w2c_decode__webp__wasmsimd__sse2_malloc(&inst, data_size);

  w2c_decode__webp__wasmsimd__sse2_SetupWebpDecode(&inst, webp_file, data_size, config);
  ...
  w2c_decode__webp__wasmsimd__sse2_DecodeWebpImage(&inst, webp_file, data_size, config);
  const double dt = StopwatchReadAndReset(&stop_watch);
  fprintf(stderr, "Time to decode pictures: %.10fs\n", dt);
  fprintf(out_time, "%f\n", dt);

  w2c_decode__webp__wasmsimd__sse2_CleanupWebpDecode(&inst, config);
  /////////////////////////////////////////////////////////////////////
EXIT:
  fclose(out_time);
  free((void*)data);
  /* Free the inst module. */
  wasm2c_decode__webp__wasmsimd__sse2_free(&inst);
